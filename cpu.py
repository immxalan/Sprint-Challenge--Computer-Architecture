...
class CPU:
    ...
    def add(self, num1, num2): # recursive
        if num2 <= 0:
            return num1
    ...
    def subtract(self, num1, num2):
        if num2 <= 0:
            ...
            return self.subtract(num1 ^ num2, (~num1 & num2) << 1)
    ...
    def ALU_ADD(self, reg_a, reg_b):
        ...
        self.MDR = self.add(self.MDR, self.REG[self.MAR])
    ...
    def ALU_ADDI(self, reg, value):
        ...
        self.MAR = reg
        self.MDR = self.add(self.REG[self.ram_read(self.MAR)], self.MDR)
        self.MDR = self.MDR & 0xFF 
    ...
    def ALU_DIV(self, reg_a, reg_b):
        ...
        if self.MDR == 0:
            ...
            sys.exit(1)
        self.MAR = self.ram_read(reg_a)
        self.MDR = self.div(self.REG[self.MAR], self.MDR)
        self.MDR = self.MDR & 0xFF 
        self.REG[self.MAR] = self.MDR
    ...
    def ALU_MUL(self, reg_a, reg_b):
        ...
        self.MDR = self.REG[self.MAR]
        self.MAR = self.ram_read(reg_a)
        self.MDR = self.mul(self.REG[self.MAR], self.MDR)
        self.MDR = self.MDR & 0xFF #
        self.REG[self.MAR] = self.MDR
    ...
    def ALU_SUB(self, reg_a, reg_b):
        ...
        self.MDR = self.REG[self.MAR]
        self.MAR = self.ram_read(reg_a)
        self.MDR = self.subtract(self.REG[self.MAR], self.MDR)
        self.MDR = self.MDR & 0xFF
        self.REG[self.MAR] = self.MDR
    ...
    def handle_CALL(self, ops):
        self.MAR = self.add(self.PC, ops)
        self.SP = self.subtract(self.SP, 1)
        if self.SP == self.PC:
    ...
    def handle_JMP(self, ops):
        ...
        # and transfers flow of execution by changing instruciton pointer register
        self.MAR = self.ram_read(self.PC + 1)
        self.PC = self.REG[self.MAR]
